...
<div class="relative flex flex-col items-center gap-8 p-2 lg:px-64">
    {#if loading}
        <div class="fixed bottom-0 left-0 right-0 top-0 z-20">
            <LoadingOverlay />
        </div>
    {/if}
    <a href="/" class="gradient-heading h1 mt-8 font-bold">MuseGraph</a>
    <div class="h-16">
        {#if $spotifyUserStore}
            <SpotifyProfile
            ...
            />
        {:else}
            <div class="card flex max-w-md flex-col items-center gap-4 p-4">
                <p>In order to create playlists, you need to log in with spotify.</p>
                <SpotifyLoginButton text="Log in with spotify" redirectUrl="/search" />
            </div>
        {/if}
    </div>
    <div class="mt-20">
        <form
            class="input-group input-group-divider max-w-md grid-cols-[auto_1fr_auto]"
            method="POST"
            use:enhance={() => {
                loading = true;
                return async ({ update }) => {
